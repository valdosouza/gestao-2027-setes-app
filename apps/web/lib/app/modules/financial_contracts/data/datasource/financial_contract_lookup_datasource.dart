...
/// módulo gêmeo (/api/financial-contracts/payment-types e /bank-accounts).
/// Regra: a page só toca datasource de lookup; o módulo fala SÓ com o seu
/// /api/financial-contracts (nunca importa payment_types/bank_accounts).
abstract class FinancialContractLookupDatasource {
...
class FinancialContractLookupDatasourceImpl
    implements FinancialContractLookupDatasource {
  const FinancialContractLookupDatasourceImpl({required this.client});
  ...
  Future<List<PaymentTypeLookup>> paymentTypes(String filter) async {
    final json = await client
        .get('/api/financial-contracts/payment-types${_query(filter)}');
  ...
  Future<List<BankAccountLookup>> bankAccounts(String filter) async {
    final json = await client
        .get('/api/financial-contracts/bank-accounts${_query(filter)}');
    final data = json['data'] as List<dynamic>? ?? [];
    return data
