...
class ServiceListListState extends ServiceListState {
  const ServiceListListState({
    this.items = const [],
    this.loading = false,
    this.filter = '',
    this.page = 1,
  ...
  final List<ServiceListEntity> items;
  final bool loading;
...
class ServiceListFormState extends ServiceListState {
  const ServiceListFormState({this.editing, this.saving = false});
  final ServiceListEntity? editing;
  final bool saving;
...
class ServiceListActionSuccess extends ServiceListState {
  const ServiceListActionSuccess(this.messageKey);
...
class ServiceListActionFailure extends ServiceListState {
  const ServiceListActionFailure(this.failure);
