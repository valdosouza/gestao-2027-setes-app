...
class BankListState extends BankState {
  const BankListState({
    this.items = const [],
    this.loading = false,
    this.filter = '',
...
class BankFormState extends BankState {
  const BankFormState({this.editing, this.saving = false});
  final BankEntity? editing;
  final bool saving;
...
class BankActionSuccess extends BankState {
  const BankActionSuccess(this.messageKey);
...
class BankActionFailure extends BankState {
  const BankActionFailure(this.failure);
