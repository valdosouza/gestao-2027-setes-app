...
import '../widget/provider_tab.dart';

/// Tela de Fornecedores — interface 'providers' (1 interface = 1 módulo,
/// ARQUITETURA_MODULOS.md). Onda 3 da Entidade Única
/// (prompt_onda3_provider.md): espelho do carrier da Onda 2 — form com
/// 6 abas: 5 COMPARTILHADAS (shared/entity/widgets, incluindo EntityTaxTab
/// — D1) + a específica ProviderTab.
///
/// O bloc guarda o DRAFT do ObjectProvider inteiro; as abas editam fatias
...
class ProviderPage extends StatefulWidget {
  const ProviderPage({required this.title, super.key});
...
class _ProviderPageState extends State<ProviderPage> {
  ...
  final _formViewKey = GlobalKey<_ProviderFormViewState>();

  @override
  void initState() {
    super.initState();
    _bloc = Modular.get<ProviderBloc>()..add(const ProviderListRequested(''));
  ...
  Widget _buildSearch(ProviderListState state) =>
      RegisterSearchPage<ProviderListItem>(
        title: 'register.listTitle'.tr(args: [widget.title]),
        items: state.items,
        loading: state.loading,
        // Engrenagem padrão da lista (Framework de Configurações, decisão 11)
        configModuleKey: 'providers',
        avatarBuilder: (p) => '${p.id}',
        rowBuilder: (p) => [
          p.nickTrade ?? p.nameCompany ?? '',
          p.nameCompany ?? '',
          p.active
              ? 'forms.provider.active'.tr()
              : 'forms.provider.inactive'.tr(),
        ],
  ...
  Widget _buildForm(ProviderFormState state) => _ProviderFormView(
        key: _formViewKey,
        title: widget.title,
        draft: state.draft,
        ...
        onDraftChanged: (draft) => _bloc.add(ProviderDraftChanged(draft)),
        onSave: () => _bloc.add(ProviderSaveRequested(
            draft: state.draft, creating: state.creating)),
        onBack: () => _bloc.add(const ProviderBackToListPressed()),
        onDelete: state.creating || state.draft.id == null
            ? null
            : () => _bloc.add(ProviderDeleteRequested(state.draft.id!)),
      );
  ...
  Future<void> _askDuplicateRole(int existingId) async {
    final decision = await askDecision(
      context,
      message: 'forms.provider.duplicateRole'.tr(),
      yesLabel: 'forms.provider.openEdit'.tr(),
    );
    if (decision == SetesDecision.yes && mounted) {
      _bloc.add(ProviderEditPressed(existingId));
    }
  }

  @override
  Widget build(BuildContext context) =>
      BlocConsumer<ProviderBloc, ProviderBlocState>(
        bloc: _bloc,
        listenWhen: (_, current) =>
            current is ProviderActionSuccess ||
            current is ProviderActionFailure ||
            current is ProviderDuplicateRole,
        ...
        listener: (context, state) {
          if (state is ProviderDuplicateRole) {
            _askDuplicateRole(state.existingId);
            return;
          }
          if (state is ProviderActionSuccess) {
            showSuccessFeedback(context, state.messageKey);
            return;
          }
          final failure = (state as ProviderActionFailure).failure;
        ...
        buildWhen: (_, current) =>
            current is ProviderListState || current is ProviderFormState,
        builder: (context, state) => switch (state) {
          ProviderFormState() => _buildForm(state),
          ProviderListState() => _buildSearch(state),
          _ => _buildSearch(const ProviderListState(loading: true)),
        },
      );
}
...
class _ProviderFormView extends StatefulWidget {
  const _ProviderFormView({
    required this.title,
    required this.draft,
  ...
  final ValueChanged<ObjectProvider> onDraftChanged;
  final VoidCallback onSave;
  final VoidCallback onBack;
  final VoidCallback? onDelete;

  @override
  State<_ProviderFormView> createState() => _ProviderFormViewState();
}

class _ProviderFormViewState extends State<_ProviderFormView>
  ...
  ObjectProvider get _draft => widget.draft;
  ...
  Widget build(BuildContext context) {
    ...
    return SetesFormShell(
      ...
      child: KeyedSubtree(
        key: ValueKey(
            widget.creating ? 'provider-new' : 'provider-${draft.id}'),
        child: Column(
          children: [
            TabBar(
              controller: _tabs,
              ...
              tabs: [
                ...
                Tab(text: 'forms.provider.tab'.tr()),
            ...
            Expanded(
              child: TabBarView(
                ...
                children: [
                  ...
                  // Aba Tributação COMPARTILHADA (D1): edita a fatia `tax`
